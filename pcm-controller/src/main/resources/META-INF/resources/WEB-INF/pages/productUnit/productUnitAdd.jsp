...
<html>
	...
	<body class="${param.skin}">
		...
		<form:form cssClass="form-horizontal" id="pcmProductUnitAddForm" cssStyle="padding-top: 40px" modelAttribute="bPcmProductUnit" method="post" action="${ctx}/productUnit/addProductUnit.in" data-confirm="true">
			...
			<div class="form-group row">
				...
				<div class="col-lg-7 col-md-7 col-sm-6 col-xs-6">
					<form:input cssClass="form-control" type="text" path="unitCode" data-rule-required="true" data-rule-maxlength="32" />
				</div>
				<div class="col-lg-3 col-md-3 col-sm-4 col-xs-4 remark">
			...
			<div class="form-controls auto-float">
				<div class="cbtn-group-md">
					<!-- 确定 -->
					<input type="button" class="btn-primary btn" id="productUnitAddBtn" value="<spring:message code='kite.web.common.btnSure' text='确定' />" />
					<!-- 返回 -->
	...
	<script type="text/javascript">
		// 开启表单验证
		$("#pcmProductUnitAddForm").validate();
		...
		$("#unitModule").change(function() {
			...
			if (unit) {
				...
				$("#unitCode").val(unit.unitCode);
				$("#unitName").val(unit.unitName);
				$("#unitNameCn").val(unit.unitNameCn);
			} else {
		...
		// 提交
		$("#productUnitAddBtn").on('click', function() {
			if ($("#pcmProductUnitAddForm").valid()) {
				let dataMap = $('#pcmProductUnitAddForm').getFormData();
				dataMap['unitRelations'] = $K.JSON.stringify(ascSlt.getValidAssemble());
				$K.ajax({
					url: '${ctx}/productUnit/addProductUnit.in',
					type: 'post',
					data: dataMap,
					success: function() {
