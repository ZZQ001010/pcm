...
<body class="${param.skin}">
			...
			<div class='panel panel-default'>
				...
				<form:form cssClass="form-horizontal" id="productUpdForm" cssStyle="padding-top: 20px" modelAttribute="product" method="post" action="${ctx}/product/updProduct.in" data-confirm="true">
					...
					<div class="form-group row">
						...
						<div class="col-lg-2 col-md-4 col-sm-4 col-xs-4">
							<form:input cssClass="form-control"  path="cardnoRangeCeil" data-rule-required="true" data-rule-maxlength="12" data-rule-digits="true" />
						</div>
						<!-- 电子账号上限描述 -->
						<div class="hide desc">
							<spring:message code="product.cardnoRangeCeil.desc" />
						</div>
						<!-- 电子账号下限 -->
						<label class="col-lg-2 col-md-2 col-sm-2 col-xs-2 control-label">
							<span class="span-icon">*&nbsp;</span>
						...
						<div class="col-lg-2 col-md-4 col-sm-4 col-xs-4">
							<form:input cssClass="form-control"  path="cardnoRangeFlr"  data-rule-maxlength="12" data-rule-digits="true" data-rule-amountLimit="true"/>
						</div>
						<!-- 电子账号下限描述 -->
						<div class="hide desc">
							<spring:message code="product.cardnoRangeFlr.desc" />
						</div>
					</div>
					<div class="form-group row">
						<!-- 卡号长度 -->
						...
						<!-- 卡号长度描述 -->
						<div class="hide desc">
							<spring:message code="product.cardnoLen.desc" />
						</div>
						<!-- 基准货币 -->
						<label class="col-lg-2 col-md-2 col-sm-2 col-xs-2 control-label">
							<span class="span-icon">*&nbsp;</span>
							...
							<div class="stats-container">
								<div class="unit-title" style="text-align: left;">
									...
									<div class='product content-opacity'>
										<i class='product-view fa fa-list-ul' title="查看"  data-target-url="${units.unitDetailUrl }" data-param-key='${dataMap[units.unitCode]}' ></i>
										<i class='product-config fa fa-wrench' title="配置" data-target-url="${units.unitConfigUrl }" data-iframe-id="${units.unitCode }" data-config-index="${units.unitIndex }"
										 data-unit-class='${units.unitClass }' data-param-key='${dataMap[units.unitCode]}' 
										 data-relation-untis="${units.updateUnits }"  data-config-page="${units.unitConfig }" 
								...
								<div class="unit-content content-opacity" >
									...
									<iframe id="iframe_${units.unitCode }" data-oldbase-url='${ctx }${units.unitBaseUrl }'  src="${ctx }${units.unitBaseUrl }?productCode=${dataMap[units.unitCode]}&subUnit=${units.subUnit}"  style="min-height: 240px;max-height: 240px" width="100%" height="0px" frameborder="no"  border="0" marginwidth="0" marginheight="0"  allowtransparency="yes"></iframe>
								</div>
							</div>
						</div>
					</c:forEach>
	...
	jQuery.validator.addMethod("amountLimit", function(value, element) {
		...
		return false;
}, "<spring:message code='product.accountCompare' text='电子账号下线必须小于电子账号上限,且必填' />");
		...
		$(".product-config").each(function(i,view){
			...
			if(paramKey){
				configMap[index]=true;
				configCount++;
			    var divCont=$(this).parents(".stats-container");
			    divCont.css("border","1px solid rgba(76,140,245,0.52)");
			    //将数据都添加到productDatas
			    var oldProMap={};
			    oldProMap.productCode="${product.productCode}";
			    oldProMap.productType="${product.productType}";
			    oldProMap.unitCode=$(this).data("iframe-id");
			    oldProMap.paramClass=$(this).data("unit-class");
			    oldProMap.paramKey=paramKey;//参数配置页面的paramKey
			    productDatas.push(oldProMap);
			    //将content-opacity样式移除
			    $(this).parents(".stats-container").children().each(function(i,view){
			    ...
			    $("i[data-config-index='"+(index+1)+"']").parent().removeClass("content-opacity");
			...
			$(this).click(function(){
				...
				var targetUrl=configObj.data("target-url")+"?productCode="+productCode;
				//判断页面是否是子页面
				...
				var unitClass=configObj.data("unit-class");
				var relationUnitCodes=configObj.data("relation-untis");//iframe关联 需要关联的iframe的id
				//判断是否需要联动查询数据
				if(paramsMap[iframeId]){
					targetUrl=configObj.data("target-url")+"?"+paramsMap[iframeId].join("&");
				}
				//获取弹窗配置大小
