...
(function(factory) {
  ...
  function attachRel(data, flags) {
	  //@author zzq  遇到children 先进行转换 
	if(data.children==null || data.children=="null"){
		data.children=[]
	}
  ...
  // @auther zzq 改动下获取的节点值
  //----------------- 这个地方代码写死不行哈，我修改下
  function loopChart($chart) {
    var $tr = $chart.find('tr:first');
    var subObj = { 'id': $tr.find('.node')[0].id,'unitParamId':$($tr.find('.node')[0]).children('.unitParamId').html(),'value': $($tr.find('.node')[0]).children('.title').html(),'unitNameCn': $($tr.find('.node')[0]).children('.content').html(),'unitCode': $($tr.find('.node')[0]).children('.unitCode').html() };
  ...
  function createNode(nodeData, level, opts) {
    ...
    var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (nodeData[opts.nodeId] ? ' id="' + nodeData[opts.nodeId] + '"' : '') + (nodeData.parentId ? ' data-parent="' + nodeData.parentId + '"' : '') + '>')
      ...
      .append(typeof opts.parentDesc !== 'undefined' ? '<div class="content">' + (nodeData[opts.parentDesc] || '') + '</div>' : '')
    ...
    $nodeDiv.on('contextmenu', function(event) {
