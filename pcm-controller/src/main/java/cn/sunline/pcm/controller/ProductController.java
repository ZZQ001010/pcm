...
public class ProductController {
	...
	public ModelAndView productQueryPage(HttpServletRequest request) throws FlatException{
		try {
			ModelAndView view = KW.mvc.forwardView("product/productQuery");
	...
	public ModelAndView productDetailPage(String productCode,String code,ModelAndView view) throws FlatException {
		try {
			//语言环境
			Locale locale = LocaleContextHolder.getLocale();
			boolean isChinese = Locale.CHINESE.getLanguage().equals(locale.getLanguage());
			view = KW.mvc.forwardView("product/productDetail");
			view.addObject("factory", productCode == null);
			Product product = parameterSurface.getParameterObject(
					productCode==null?code:productCode, Product.class);
			//根据产品类型查到产品大类已经大类下的产品和子产品
			List<BPcmProductGroup> groupList = parameterSurface.getProGroup(product.getProductType().name());
   ...
   public Map<String,String> unitConfig() throws FlatException{
           try{
                    ...
                    parameterSurface.getParameterObject(Product.class).forEach(
                                    item->resMap.put(item.getProductCode(),item.getDescription()));
                   return resMap;
           } catch (ProcessException e) {
                   logger.error(e.getMessage(), e);
                   throw new FlatException(e.getErrorCode(), e.getMessage());
   ...
   public List<String> unitBase(@RequestParam("code") String code){
	   Product parameterObject = parameterSurface.getParameterObject(code,Product.class);
                   List<String> list = new ArrayList<>();
                   list.add(parameterObject.getProductCode());
                   list.add(parameterObject.getDescription());
                   list.add(KC.Enum.getI18nLabel(parameterObject.getProductType()));
                   return list;
   }
